...
class GeneraChiavi {
  int cont = 0;
  var myFile = File('./lib/src/rsa/primi.txt');
  String pathnew = Directory.current.createTempSync().path;
  //variabili per rsa
  var p = BigInt.from(0);
  var q = BigInt.from(0);
  var e = BigInt.from(0);
  ...
  //link al file
  generaChiavi() {
    myFile.copySync('$pathnew/primi.txt');
    myFile = File('$pathnew/primi.txt');
    ...
    myFile.openRead().transform(utf8.decoder).transform(const LineSplitter());
